...
typedef CPnum   (* FNTYPE(M_MNNSm,r64,RM) )(CPnum m0, Tsize nc0, Tsize nr0, Tnum s0, Pnum m);
...
typedef Tenum   (* FNTYPE(E_mNNm,r64,RM))(Pnum m0, Tsize nc0, Tsize nr0, Pnum m1);
// vector equality discrimination
typedef Tenum   (* FNTYPE(E_MMNS,r64,RM))(CPnum m0, CPnum m1, Tsize nc0, Tnum s0);
// vector cross product
typedef CPnum   (* FNTYPE(M_MMm,r64,RM))(CPnum m0, CPnum m1, Pnum m0xm1);
// vector normalize
typedef CPnum   (* FNTYPE(M_MmN,r64,RM))(CPnum m0, Pnum m, Tsize ndim);
typedef struct {
    ...
    const FNTYPE(M_MNNSm,r64,RM)   muls; // multiply matrix and scalar number
    ...
    const FNTYPE(E_mNNm,r64,RM) soldense; // solve dense linear equation
    const FNTYPE(E_MMNS,r64,RM) equalv; // 1: m0 == m1 within s0 tolerance
    const FNTYPE(M_MMm,r64,RM) crossproduct;   // cross product of two vectors which is 1x3 or 3x1 matrices.
    const FNTYPE(M_MmN,r64,RM) normalize;   // normalize a vector
